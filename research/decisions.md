...
## D21 - Track A is paused because the repository has run out of non-circular observations

**Date**: 2026-08-25 · **Origin**: the post-AE-1 component survey

**The decision**: no further synthetic-benchmark experiments. Not because the research is exhausted,
but because **the current repository has reached the limit of what it can legitimately teach without
external observations.**

**The reasoning, stated as a filter.** A component can support a research question only if it has
(a) an observable target that is *not generated by the thing being evaluated*, and (b) either an
oracle or enough data to measure the target. Applying that across the codebase:

| Component | Verdict |
|---|---|
| Matchup engine | Exhausted at ~81 balls/batter (nine experiments) |
| Win probability | Closed by AE-1 - the gap was functional form, not representation |
| `rainRuleCalculator` | Measurable against published DLS output, but the error is a curve-fitting problem, not an algorithmic one |
| `mvpCalculator` | Target is generated by the algorithm being evaluated - circular in current data |
| `keyMoments` | No ground truth on "key"; measurable only *via* the win-probability oracle, so any finding is a corollary of AE-1 |
| Story / commentary / article generators | No target exists |
| `predictionSettler`, notifications, charts | Deterministic rules; nothing is predicted |

**Why the AI engine was researchable and nothing else is.** It had two properties nothing else has:
an observable target already present in the data (match outcome), and an **exact oracle**, available
only because `matchSimulator.js` is i.i.d. Remove either and E6/AE-1 are unrunnable. This is
structural, not a failure of imagination.

**What would unpause Track A**: real observations. Specifically the two assets identified by the
survey, both of which are currently empty and both of which are created by *pilot adoption rather
than by analysis* -

1. **Human Man-of-the-Match overrides** - `algorithm prediction -> human decision -> outcome`. The
   only place in the system where a human judgment is recorded against an algorithmic one.
2. **User match predictions** (`Prediction`) - `human forecast -> outcome`, independent of any model.

Together these support a class of question the simulator cannot pose at all: **when do humans and
the model disagree, who is right when they do, and can a system learn when to defer?** That is a
decision-support question, not a leaderboard question, and it generalises well beyond cricket.

**Falsification**: if a component is found that satisfies the filter above on data the system does
