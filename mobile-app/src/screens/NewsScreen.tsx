...
export default function NewsScreen() {
  ...
  const load = useCallback(() => {
    setError(null);
    const request = mode === 'mine' && user ? api.news.getFeed() : api.news.getPosts();
    request
    ...
    <View style={styles.container}>
      {user ? (
        <View style={styles.toggleRow}>
          <TouchableOpacity
            style={[styles.toggleButton, mode === 'public' && styles.toggleButtonActive]}
            onPress={() => setMode('public')}
          >
            <Text style={[styles.toggleText, mode === 'public' && styles.toggleTextActive]}>All News</Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={[styles.toggleButton, mode === 'mine' && styles.toggleButtonActive]}
            onPress={() => setMode('mine')}
          >
            <Text style={[styles.toggleText, mode === 'mine' && styles.toggleTextActive]}>My Tournaments</Text>
          </TouchableOpacity>
        </View>
      ) : null}
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <Text style={styles.muted}>
              {mode === 'mine' ? 'No news for your tournaments yet.' : 'No news yet.'}
            </Text>
          }
          renderItem={({ item }) => {
            ...
            return (
              ...
              >
                ...
                {(item.tournament || item.isAutoGenerated) && (
                  <View style={styles.tagsRow}>
                    {item.tournament && (
                      <View style={styles.tournamentBadge}>
                        <Ionicons name="trophy-outline" size={11} color={colors.pitch400} />
                        <Text style={styles.tournamentBadgeText}>{item.tournament.name}</Text>
                      </View>
                    )}
                    {item.isAutoGenerated && (
                      <View style={styles.matchReportBadge}>
                        <Ionicons name="stats-chart-outline" size={11} color={colors.gold400} />
                        <Text style={styles.matchReportBadgeText}>Match Report</Text>
                      </View>
                    )}
                  </View>
                )}
...
const styles = StyleSheet.create({
  ...
  toggleButton: {
    flex: 1, backgroundColor: colors.surfaceAlt, borderRadius: 999, paddingVertical: 9, alignItems: 'center',
  },
  toggleButtonActive: { backgroundColor: colors.pitch500 },
  toggleText: { color: colors.inkSecondary, fontSize: 13, fontWeight: '600' },
  toggleTextActive: { color: '#06170D' },
  ...
  tournamentBadge: {
    flexDirection: 'row', alignItems: 'center', gap: 4, backgroundColor: colors.pitch900,
    borderRadius: 999, paddingHorizontal: 8, paddingVertical: 3,
  },
  tournamentBadgeText: { color: colors.pitch400, fontSize: 11, fontWeight: '600' },
  matchReportBadge: {
    flexDirection: 'row', alignItems: 'center', gap: 4, backgroundColor: colors.surfaceAlt,
    borderRadius: 999, paddingHorizontal: 8, paddingVertical: 3,
  },
  matchReportBadgeText: { color: colors.gold400, fontSize: 11, fontWeight: '600' },
